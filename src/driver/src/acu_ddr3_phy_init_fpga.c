...
//axi_addr[30:0] =  {row[15:0], bank[2:0], col[9:0], 2'd0} = 2GB
...
#if 0
    //axi_addr[31:0] =  {row[15], 1'd0, row[14:0], bank[2:0], col[9:0], 2'd0} 
    //                                               
    ...
    //row_b15, b14, b13, b12, internal base from 18 to 21
     WRITE32(          ADDRMAP6, 0x08070707);    
     // row15 set to soc address bit[31], which can be set by mpu system management register, 
     // work around for fpga single channel, which can be used as share-AC dual channel(fpga =2GB, 1st 1GB for axi port0, 2nd 1GB for axi port1) 
     //
     //row_b17, b16, no used
...
void Load_PE_SDRAM_init(void)
{
...
#ifdef DDRC_FPGA_SIM 
	WRITE32(PUB_ADDR_BASE+0x0810,0x00000100);   // 2us
#else
...
#ifdef DDRC_FPGA_SIM 
	WRITE32(PUB_ADDR_BASE+0x0830,0x00000100);
#else 
	...
	WRITE32(PUB_ADDR_BASE+0x0850,0x0000003e);   // delay for load mode after cke 
...
#ifdef DDRC_FPGA_SIM 
	WRITE32(PUB_ADDR_BASE+0x08e0,0x880000c3);//MR1 DLL enable   bit[8] = 0 for simulation
#else
	WRITE32(PUB_ADDR_BASE+0x08e0,0x880001c3);//MR1 DLL off      bit[8] = 1 for fpga platform
#endif
	...
	WRITE32(PUB_ADDR_BASE+0x0920,0x800320c3);   //MR0 BL8, 
...
void acu_pub_phy_fpga_init(int DPHY_ADDR_BASE)
{
	...
	/*WRITE_PIPE_DELAY*/
	//WRITE32(PUB_ADDR_BASE+0xC,0x06);    // for CWL=5
	WRITE32(PUB_ADDR_BASE+0xC,0x28);    // for CWL=5
	/*CA_PIPE_DELAY*/
	WRITE32(PUB_ADDR_BASE+0x10,0x1);
	/*DFI_READ_DELAY_RANK_0*/
	//WRITE32(PUB_ADDR_BASE+0x30,0xFFFF);//Byte0~Byte7, simulation training passed setting
	WRITE32(PUB_ADDR_BASE+0x30,0xAAAA);//Byte0~Byte7, ??
	/*FCLK_READ_DELAY_RANK_0*/
	WRITE32(PUB_ADDR_BASE+0x34,0x44444444);//Byte0~Byte7,  after training , offset will set to -2, so is 4-(-2)
	//WRITE32(PUB_ADDR_BASE+0x34,0x66666666);//Byte0~Byte7, simulation training passed setting
	/*DFI_READ_DELAY_RANK_1*/
	//WRITE32(PUB_ADDR_BASE+0x40,0xFFFF);//Byte0~Byte7,DFICLK delay for dfi_rddata_en = 0x3 should set same as DFI_READ_DELAY_RANK_0
	...
	WRITE32(PUB_ADDR_BASE+0x44,0x44444444);//Byte0~Byte7, fclk delay for read data pipe = 0x6
...
void acu_pub_phy_fpga_training(void)
{
	...
	/*DATA_BYTE_SEL
	RANK: one CS
	Highest Numbered Dbyte: 0x3
	*/
	WRITE32(PUB_ADDR_BASE+0x10,0x03);
...
//    but de-asserted prior to PLAYBACK_ENABLE bit is set
	...
	/*wait training pass or fail*/
	readVal=READ32 ( PUB_ADDR_BASE+0x64);
	while(1) {
        if(((readVal & 0x1) == 0x1) || ((readVal & 0x4) == 0x4))  break;
        else readVal=READ32 ( PUB_ADDR_BASE+0x64); 
    }

	//check training pass or fail
	if(readVal&0x4) {
        //check FCLK_READ_OFFSET_RANK_0_BYTES_7_0
	    readVal=READ32 ( PUB_ADDR_BASE+0x50);
        DEBUG_MSG("FPGA phy training fail \n");
    }
	else {
        DEBUG_MSG("FPGA phy training pass \n");
    }
